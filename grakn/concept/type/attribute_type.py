...
from grakn.common.exception import GraknClientException, INVALID_CONCEPT_CASTING
from grakn.common.label import Label
from grakn.common.rpc.request_builder import attribute_type_get_owners_req, attribute_type_put_req, \
...
class _AttributeType(AttributeType, _ThingType):
    ...
    def as_boolean(self):
        ...
        raise GraknClientException.of(INVALID_CONCEPT_CASTING, BooleanAttributeType.__name__)

    def as_long(self):
        if self.is_root():
            return _LongAttributeType(self.ROOT_LABEL, is_root=True)
        raise GraknClientException.of(INVALID_CONCEPT_CASTING, LongAttributeType.__name__)

    def as_double(self):
        if self.is_root():
            return _DoubleAttributeType(self.ROOT_LABEL, is_root=True)
        raise GraknClientException.of(INVALID_CONCEPT_CASTING, DoubleAttributeType.__name__)

    def as_string(self):
        if self.is_root():
            return _StringAttributeType(self.ROOT_LABEL, is_root=True)
        raise GraknClientException.of(INVALID_CONCEPT_CASTING, StringAttributeType.__name__)

    def as_datetime(self):
        if self.is_root():
            return _DateTimeAttributeType(self.ROOT_LABEL, is_root=True)
        raise GraknClientException.of(INVALID_CONCEPT_CASTING, DateTimeAttributeType.__name__)
