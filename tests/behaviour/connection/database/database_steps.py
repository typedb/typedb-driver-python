...
def delete_databases(context, names: List[str]):
    for name in names:
...
def delete_databases_throws_exception(context, names: List[str]):
    for name in names:
        try:
            context.client.databases().delete(name)
            assert False
...
def has_databases(context, names: List[str]):
...
def does_not_have_databases(context, names: List[str]):
