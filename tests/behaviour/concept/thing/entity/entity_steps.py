...
def step_impl(context: Context, type_label: str, var: str):
    assert_that(context.tx().concepts.get_entity_type(type_label).get_instances(context.tx()), has_item(context.get(var)))


@step("entity({type_label}) get instances is empty")
def step_impl(context: Context, type_label: str):
    assert_that(calling(next).with_args(context.tx().concepts.get_entity_type(type_label).get_instances(context.tx())), raises(StopIteration))
