...
def step_impl(context: Context, root_label: RootLabel, type_label: str):
    attribute_labels = [Label.of(s) for s in parse_list(context.table)]
...
def step_impl(context: Context, root_label: RootLabel, type_label: str):
    ...
    actuals = list(map(lambda t: t.get_label(), context.get_thing_type(root_label, type_label).as_remote(context.tx()).get_plays()))
